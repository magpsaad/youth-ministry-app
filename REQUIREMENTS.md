...
| 5+ (terminal) | none (permanent, merged) | admin-set, e.g. "Yr 5+" | Normal group access rules |

**How this works mechanically (revised during Phase G build, after owner review):**
...
- **The terminal tier (Yr 5+) is a single, permanent group row — not one row per graduated cohort.** There is never a need to distinguish which original cohort a terminal member came from, not even for archiving — members leave the terminal tier one at a time, on a case-by-case basis, as they voluntarily stop attending; there is never a "whole cohort" archive action. (The only thing that might ever need filtering by original cohort in the future is a member's own `date_of_birth`, not the group they're in — not a current requirement.)
- On each transition, the cohort currently at position 4 **merges into the terminal group**: its members' `group_id` is reassigned to the terminal row, its own now-empty row is archived (not deleted, for history), and its own QR code is deleted (the terminal group's own permanent QR is used going forward). The terminal group's `cohort_year`/`name` are therefore **fixed and admin-set** — never regenerated from a shifting cohort_year, since after the first merge there's no longer one single cohort that describes it.
- **Servants/Sub-Coordinators/Read-Only grants scoped to the outgoing cohort do NOT follow their members into the terminal tier** (owner-confirmed correction to an earlier draft) — they roll back to the freshest incoming cohort instead (the group that was position 0, becoming position 1 in this same transition). Servant effort is better redirected to the newest cohort than kept on one that's now in the less-actively-managed terminal tier.
- **QR color (revised)**: Years 1-4 keep their own already-assigned color unchanged as they climb the ladder (existing behavior). The terminal group's own color is **not** permanent — each transition, it updates to the color of whichever cohort just merged in (since the terminal tier's population becomes dominated by whoever most recently joined it, as older members trail off individually over time). The brand-new incoming Yr 0 cohort inherits the terminal group's color **as it was immediately before that update** — i.e., each transition relays the terminal color one step down onto the new Yr 0, then updates the terminal color to the newly-merged cohort's.
...
When the group-transition trigger fires (§5), **Sub-Coordinators and Servants scoped to a group move with their cohort** for positions 1-4 — their `group_id` follows the same underlying group row as it advances ladder position, exactly like a Servant's member assignments do. **Exception, revised during Phase G build**: for the cohort merging from position 4 into the terminal tier specifically, its Servants/Sub-Coordinators/Read-Only grants do **not** follow their members into the terminal group — they roll back to the freshest incoming cohort (the group that was position 0, becoming position 1 in this same transition) instead. See §2.2/§5.
...
- **Effect of one click**: groups at positions 1→2, 2→3, and 3→4 each simply advance in place (same row, `ladder_position` incremented, name regenerated). The cohort at position 4 **merges into the single, permanent terminal (Yr 5+) group** instead of becoming a new row at position 5 (§2.2, revised during Phase G build after owner review) — its members' `group_id` moves to the terminal row, its own row is archived, its own QR code is deleted. Its Servants/Sub-Coordinators/Read-Only grants roll back to the freshest incoming cohort (§4.3) rather than following into the terminal tier.
- **QR color relay (added)**: Years 1-4 keep their own color unchanged. The terminal group's color updates to match the cohort that just merged into it; the brand-new Yr 0 cohort inherits the terminal group's color as it was immediately before that update (§2.2).
...
**New incoming cohort's color and the terminal group's own color (built in Phase G, revised from the original plan after owner review)**: the terminal (Yr 5+) group is a single permanent row (§2.2), and its own color is **not** fixed — each transition it updates to the color of whichever cohort just merged into it from position 4 (since the terminal tier's population is dominated by whoever most recently joined). The brand-new incoming Yr 0 cohort inherits the terminal group's color **as it was immediately before that update** — the color relays one step down onto the new Yr 0, then the terminal color updates to the newly-merged cohort's. Implemented in `run_group_transition()` (migration 0028).
