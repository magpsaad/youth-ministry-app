...
- **Yr 4's row *becomes* the new terminal row, exactly like every other position advances** (owner: "think of it as Yr 4 becomes Yr 5, just like all the younger years... it's the Yr 5+ cohort being added to the Yr 4," not the other way around) — same row, `ladder_position` set to 5, its own already-assigned `qr_color` left untouched, and its name regenerated as `"{its cohort_year} and earlier - Yr 5+"` (the aggregate pattern) rather than the normal 1-4 template. Its own Servants/Sub-Coordinators/Read-Only grants naturally stay attached — no reassignment needed, they just follow their row like any normal advance.
...
- **Effect of one click**: groups at positions 0→1, 1→2, 2→3, and 3→4 each simply advance in place (same row, `ladder_position` incremented, name regenerated from the template). The row at position 4 **also just advances in place, to position 5** (§2.2, revised during Phase G build after three rounds of owner review) — same row, same color, just its name regenerated using the aggregate pattern instead of the normal template. The row that *was* the terminal group is what's absorbed and archived: its members' `group_id` moves into the row that just became the new terminal, its own row is archived, its own QR code is deleted, and its Servants/Sub-Coordinators/Read-Only grants roll back to the freshest incoming cohort (§4.3) rather than following their (now-elsewhere) members.
