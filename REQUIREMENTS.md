...
- `id`, `user_id` (FK → users), `role` (`admin` | `general_coordinator` | `sub_coordinator` | `servant`), `group_id` (FK → groups, **required** for `sub_coordinator` and `servant`, **null** for `admin`/`general_coordinator`)
...
- **"Overview" box**: Total [Member label]s, Never Attended, Present Last Service, Absent Last Service — wrapped in a card styled identically to the "Proximity" box below it, each with its own visitor-exclusion disclaimer underneath.
...
- **Member detail** (opened as a client-side modal overlay, matching the current app's actual UX — no per-record URL): the member's name is the modal's title, so there's no separate read-only "Full Name" field cluttering the form. A **Visitor** checkbox sits at the top (editable in edit mode). Photo: click-to-upload/replace/delete (camera or file picker; no cropping in this pass — a reasonable follow-up polish item). Phone, Email, University/Affiliation, Program, DOB, Father of Confession, Home Address, Registration Comments (always read-only), Assigned Servant (gender-**soft-sorted**, not hard-filtered, dropdown with caseload counts — distinct from the New Registrations widget's hard filter, §6.3), Servant Comments. Edit mode highlights editable fields with the pale-yellow/amber styling described in §8. Buttons: Edit → Save/Cancel/Close, plus "Prev. Outreach" (lists all past entries) / "New Outreach" (always prefills the current user as servant, §6.6) shortcuts.
- General Coordinator/Admin only: **permanent delete** action for a mistaken/test record (§3.3.1), with confirmation — not visible to Servants at all.
...
- **9 PM cutoff rule** (now configurable — see §7.2): preserve the same-day gating behavior, using the deployment's configured cutoff time and timezone.
...
### 6.14 Admin screens
- **Access Maintenance**: manage who holds which role(s), scoped to which group(s), over `user_roles` (§4). **Example of an "exception" grant**: a servant serving the 2007 Cohort who also needs visibility into the 2005 Cohort simply gets a second role row here — e.g., an additional Sub-Coordinator grant scoped to the 2005 Cohort — through this same screen, no separate mechanism required.
