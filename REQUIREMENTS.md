...
- **Custom feasts & fasts (added)**: Admins can add their own named rule to the computed set (`holiday_rules` table, migration 0020) -- either a **fixed** Gregorian-equivalent date, or a date **relative to that year's Pascha**, each with a configurable duration in days so a multi-day fast (not just a single feast day) can be represented as one entry. Seeded example, per the owner's explicit request: **St. Mary's Fast**, Aug 7 - Aug 21 (the 15 days preceding the Coptic Feast of the Assumption, Aug 22) -- "we don't hold meetings during those 2 weeks." Rules can be deactivated (kept but excluded from the preview/preload) or deleted; the year-N preview recomputes every active rule's actual date(s) live, so it's never stale.
...
- **Servants Attendance**: same Present/Absent pattern as member attendance (§6.5), applied to servants.
