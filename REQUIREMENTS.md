...
- **Pending Servants** (new, Coordinator Corner, Phase C build): review queue for servants who self-registered via the "Servants" QR code but don't have an app account yet (§6.11) — shows their submitted info and how many times they've checked in while pending, with an Approve action. A minimal, functional version of this screen shipped in Phase C; it's expected to get the same visual/UX polish as the rest of this section in a later pass.
