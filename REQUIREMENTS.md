...
- **Servants Attendance**: same Present/Absent pattern as member attendance (§6.5), applied to servants across the whole ministry (not scoped to one group); an Attendance % column (same rolling-12-month rule) sits before the Status column, with the window stated on-screen.
