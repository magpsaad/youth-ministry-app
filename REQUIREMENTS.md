...
- **Actions Needed &amp; Attendance Config** (renamed): per-proximity threshold editor over `actions_needed_config` (§3.10/§7.1), plus (added) the two rolling-attendance-window settings from §7.2 -- `youth_attendance_window_weeks`/`servant_attendance_window_weeks`, folded into this existing screen rather than a new one. **Known gap, not yet resolved**: the Dashboard's actual Actions Needed section is still the Phase B placeholder ("nothing to report yet") -- those specific thresholds are stored and editable but nothing reads them yet. Flagged for a future round.
...
- **Average attendance % = `present dates ÷ tracked dates in the rolling window`, floored at Join Date** — the window never reaches before the person's Join Date, whichever is later. The rolling window length is **admin-configurable, in weeks, independently for youths and servants** (`app_settings.youth_attendance_window_weeks` / `servant_attendance_window_weeks`, both starting at 52 -- owner's explicit choice, editable on the Actions Needed Config screen, §6.14). This replaces the earlier all-time-since-registration rule for members and the hardcoded rolling-12-months rule for servants -- both are now the exact same formula, just with independently configurable window lengths and both floored at the real Join Date instead of `created_at`.
