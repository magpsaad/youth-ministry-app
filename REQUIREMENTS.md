...
- **App Settings** (renamed again, was "Actions Needed & Attendance Config"; landing-page button reordered to sit after Verses Maintenance): the two rolling-attendance-window settings from §7.2 (`youth_attendance_window_weeks`/`servant_attendance_window_weeks`, blank = no cap), plus the per-proximity threshold editor over `actions_needed_config` (§3.10/§7.1), both on one screen (same route, `/admin/actions-needed-config`, just relabeled).
- **Actions Needed (§7.1 algorithm, built)**: the Dashboard's Actions Needed section (§6.3) is no longer a placeholder -- flagged members are computed live (`lib/actions-needed.ts`) against the exact §7.1 rule, grouped by assigned servant, shown as color-coded cards (photo, consecutive-absence count, last outreach date, an Outreach quick-link, and a Dismiss button). Dismissal is session-only (`sessionStorage`, same mechanism as "My Assigned List" and collapsed-section state) -- there's no schema column for a permanent dismissed state, and re-flagging each new session was the deliberate, simpler choice. The "?" help modal explaining the rule is generated from the live `actions_needed_config` values, per §6.9. **Deliberately not built (scope decision, not asked for)**: merging Actions Needed cards with open outreach follow-up reminders and new-assignment notifications into one unified per-servant list, as originally described in §5's item 6 -- outreach follow-up reminders (`outreach_entries.follow_up_due`) aren't surfaced anywhere in the UI yet at all, and building that is a separate feature; Actions Needed and New Registrations remain separate Dashboard sections for now.
