...
**App Labels & Branding (added)**: an editable form at the top of the same screen for every generic app-identity field that used to be set-once via bootstrap SQL — `app_title_long`, `app_title_short` ("Ministry Label"), **Dashboard Subtitle** (`app_subtitle`), **Group Label** (`group_label`, e.g. "Cohort"), **Member Label** (`member_label`, e.g. "Youth"), `theme_color`, **Service Day** (`service_weekday`, a day-name dropdown over the same ISO-weekday integer `is_service_day()` already used, default Friday — drives self-check-in gating §6.11, the Attendance tab's same-day cutoff §7.2, and which dates count toward average attendance % §7.2), `logo_url`. Saving revalidates the whole app (`revalidatePath("/", "layout")`), since these fields feed the header/nav shell on every page.
