...
- **Servant Directory** (read-only, from the landing page, labeled "Servant Directory" singular): searchable, with the same Categorical/Alphabetical toggle as the two screens above (grouped by serving group, then General Coordinators, then Unassigned, in Categorical mode), phone (tel: link), and average attendance %. **Attendance % is a rolling trailing 12 months** (owner's explicit choice, distinct from members' all-time-since-registration rule in §7.2) — stated at the top of the screen.
