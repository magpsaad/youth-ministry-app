...
- **Servant Assignments** (Coordinator Corner, separate screen from Servant Profiles after owner review): every servant listed with photo + name, and a second column showing their current cohort, or an "Assign" control (styled as a button/dropdown) if unassigned. Selecting a different cohort reassigns; selecting "Unassigned" unassigns. **Restricted to General Coordinators/Admins** (view-only for Sub-Coordinators, same restriction as before the split). Two view modes, **Categorical** (Unassigned listed first, then cohorts **youngest to oldest** — ladder_position ascending, Yr 0 through the terminal group, not alphabetical by name; a name like "2003 and earlier - Yr 5+" sorting before "2008 Cohort - Yr 0" was an owner-reported bug, fixed — no General Coordinators bucket here, since GC-only users don't need a cohort assignment) and **Alphabetical**, consistent with the other two screens' toggle. **Reassignment updates its own local state immediately on success** (owner-reported bug fix), rather than relying solely on `router.refresh()` to re-supply fresh props — that only works when this component is driven by a server-rendered page's own props (the standalone screen), not when it's embedded with data fetched into local client state (Group Transition's post-transition review, §5), where a dropdown pick would otherwise silently snap back to its old value even though the reassignment had actually succeeded.
