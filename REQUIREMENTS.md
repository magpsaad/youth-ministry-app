...
**App Labels & Branding (added)**: an editable form at the top of the same screen for every generic app-identity field that used to be set-once via bootstrap SQL — `app_title_long`, `app_title_short`, `app_subtitle`, `group_label`, `member_label`, `theme_color`, `logo_url` — plus the Current Birthdays date window (`birthday_window_days_before`/`birthday_window_days_after`, default 7/14, §6.3/§7.1). Saving revalidates the whole app (`revalidatePath("/", "layout")`), since these fields feed the header/nav shell on every page.

### 6.10 Group Transition tool (built in Phase G)
Admin-only (`/admin/group-transition`). A preview screen (each group's current name → next name, a note about the outgoing Yr 4 cohort merging into Yr 5+, and an editable suggested year for the new Yr 0 cohort) behind an explicit two-step confirmation, then the transition itself runs atomically via `run_group_transition()` (migration 0028) — with the new-cohort-creation, optional servant-review (reuses the Servant Assignments screen's component directly), and QR-reprint-prompt steps described in §5.
...
**3. Follow-up Due** (mauve, distinct from the amber/blue above) — `outreach_entries` rows where `follow_up_due <= today AND follow_up_dismissed_at is null`, surfaced under the **entry's own creator** (`servant_id`), not necessarily the member's currently assigned servant — this is a personal reminder the servant set for themselves. Card shows the member's photo/name, "Your follow-up is due [date]," a link that opens the original entry for editing (`EditOutreachEntryModal`), and a Dismiss button (sets `follow_up_dismissed_at`, keeping the entry itself intact as a record).
