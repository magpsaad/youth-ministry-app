...
import { getAuditLogsAction, getAuditConfigAction } from "@/app/admin/audit-logs/actions";
...
export default async function AuditLogsPage() {
  ...
  const [settings, logs, config] = await Promise.all([
    getAppSettings(),
    getAuditLogsAction({}),
  ...
  return (
    <div className="min-h-full bg-[#f5f5f5]">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        <AuditLogsInteractive initialLogs={logs} actionTypes={config.map((c) => c.action_type)} initialConfig={config} />
      </main>
    </div>
  );
}
