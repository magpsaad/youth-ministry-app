 ...
 * Servant, Date range), "+ Add Outreach Entry", Edit/Delete visible only on
 * entries the current user created (RLS enforces this regardless of what
 * the UI shows), quick Call/Text links. */
export function OutreachInteractive({
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [entries, q, servantId, dateFrom, dateTo]);
      ...
      <div className="flex flex-wrap items-center gap-2 justify-between">
        <div className="flex flex-wrap gap-2">
          ...
          <input
            ...
            className="rounded-md border border-[#ddd] px-3 py-2 text-sm focus:border-[#1e3a5f] focus:outline-none"
          />
          <span className="self-center text-sm text-[#666]">to</span>
          <input
            type="date"
            value={dateTo}
              ...
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <div className="mt-1.5 flex items-center gap-3">
                    <PhoneLink phone={entry.member_phone} className="text-xs" />
                    <TextLink phone={entry.member_phone} className="text-xs" />
                  </div>
                </div>
                {canEdit && (
